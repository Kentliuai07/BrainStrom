...
> ⚠️ **2026-06-11 阶段二改版波及说明（v3 活文件模型）**：本文 `systems.mode('free'|'structured')` 与「双模式切换」的语意将在阶段二改版——mode 改为 `docState('raw'|'optimized'|'carded')` 生命周期、顶部切换改「文章/卡片」两视图、body 大 textarea 将迁移成「段落=块」编辑器。阶段一已建成的骨架不必回头改，阶段二动工时按 `docs/阶段二开发文档-变聪明-AI功能.md` §1.2/§1.3 迁移即可。
