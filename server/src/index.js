...
【系統結構偵測】若這次對話明確出現專案的硬規格（名稱／前端框架／後端／API／資料庫／伺服器／部署方式），就在 propose 的 items 裡「額外」加一個提議：{ action:'update_spec', label:'記入結構'(≤12字), args:{ instruction: <一段 JSON 字串> } }。
JSON 字串的鍵只能用：name / frontend / backend / apis / database / server / deployMethod；只填這次明確提到的欄位（其餘省略不要填 null），apis 必須是字串陣列。
範例 args.instruction 值（注意整個是字串）："{\\"frontend\\":\\"React\\",\\"database\\":\\"PostgreSQL\\"}"。
沒有明確硬規格時，不要加 update_spec。它與其它提議（edit_text/structure 等）可並存於同一個 items 陣列，但最多 4 個。`;

// 系统身份证（L1）转成可读文字（只列已填栏位）
function formatSpec(spec) {
  if (!spec || typeof spec !== 'object') return '';
  const f = (k, v) => (v && String(v).trim()) ? `${k}：${v}` : '';
  const apis = Array.isArray(spec.apis) && spec.apis.length ? `API：${spec.apis.join('、')}` : '';
  return [f('名稱', spec.name), f('前端', spec.frontend), f('後端', spec.backend),
...
function buildSystem(note, kickoff, project) {
  ...
  const task = (kickoff ? (hasContent ? COACH_CONTENT_TASK : COACH_EMPTY_TASK) : CHAT_TASK) + SPEC_DETECT;
...
async function handleChatNote(req, res, payload) {
  ...
  try {
    ...
    const stream = anthropic.messages.stream({
      ...
      system: buildSystem(note, kickoff, payload?.project),
