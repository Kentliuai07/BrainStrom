...
## 第 0 节 · 使用者已拍板的四个决策（开发硬规则）

1. **手改永久保护**：使用者新增或改过的卡片，AI 永远不准动（永久保护，不因原文变动而被覆盖）。
...
2. **「哪些不能动」靠「根本不送给 AI」实现，没有任何资料层标记。** 它没有 `locked` 锁、没有 anchor、没有 hash、没有 version 边界。规则就是：没被点名的段落，连碰都不碰，原字串直接拼回去。→ BrainStrom 需要「手改卡永久保护」，所以**得自己加 `locked` / `source` 标记**（SBIR_NEW 没有，但 BrainStrom 的 `backend-design.md` 已经规划好了）。
...
| 资料形态 | 一大串 Markdown 字串 | ~20 张有 `type` 的卡片 | **改写：卡片阵列版合并** |
| 保护机制 | 不送 AI（无标记） | 手改卡永久保护（要标记） | **要新加：`source`/`locked` 栏位** |
| 省成本：没变就跳过 | 没有（每次都打 AI） | 原文没变就别花钱 | **要新做：原文指纹 hash gate** |
| 省成本：只送变动 | 有（extract） | 要 | **直接抄做法** |
| 提示词快取 | 有（ephemeral） | 要 | **直接抄做法** |
| 安全阀 | 有（30% 字数） | 要（防 AI 暴走） | **直接抄做法** |
| 版本可还原 | 有（单章 + 全篇两层） | 要 | **抄做法、改成卡片版** |
| Prompt 措辞 | 有（保留事实/占位/只改指定） | 要 | **直接抄措辞** |
| 模型分流省钱 | 没有（全 Sonnet） | 可选优化 | 可考虑增量用 Haiku |

一句话：**SBIR_NEW 给我们一台「只改一块、不碰其他、改太凶就退回、能还原」的好机器；缺的「自动看出原文哪里变」我们自己补，并把它从「字串版」改造成「卡片版」。**
...
### 6.1 资料模型改造（沿用 `backend-design.md` 已规划的栏位，不另起炉灶）

每张卡片（block）要有：

- `來源`（`source`）：`ai` | `manual`（手动）| `notes` | `voice` —— 区分 AI 产 vs 使用者手改。
- `鎖定`（`locked`）：true / false —— 受保护、重整时绝不动。
- `來源原文指紋`（`sourceAnchor` / `sourceHash`）：这张卡是「原文哪一段」生出来的指纹，供下次比对「这段原文有没有变」。
...
2. **差异侦测（SBIR_NEW 缺、我们新做）**：把「旧原文」和「新原文」做文字 diff，算出「哪几块原文片段是新增/变动的」。BrainStrom 的自由速记可能没有干净标题，所以建议：
   - 优先用「段落/空行」切块（每个自然段当一块），各块算指纹。
   - 比对新旧各块指纹，标出「新增块」「变动块」「没变块」。
   - 这一步**纯程式、不花 AI 钱**。

3. **组增量输入给 AI**：只把「变动/新增的原文片段 + 现有卡片清单（含哪些是 `locked`/`manual`）」交给 AI，并下死命令（抄 SBIR_NEW prompt 精神）：
   - 「绝不修改 `locked` 或 `source=manual` 的卡片。」
   - 「只针对我给你的变动片段，决定要新增哪几张卡、或更新哪几张 `source=ai` 的卡。」
   - 「没提到的卡一律原样保留。保留客观事实、占位标签原样保留。」

4. **AI 只回 patch（要新增/更新的卡）**：用 `tool_use + JSON schema` 强制 AI 回结构化结果，例如：
   ```json
   {
...
5. **本地合并（卡片版 Splice）**：在后端用程式把 patch 套到现有卡片阵列：
   - `locked` / `source=manual` 的卡 → **硬跳过，连看都不看**（双重保险：就算 AI 不听话回了它们，合并层也挡掉）。
   - `更新卡片` → 只换对应 `source=ai` 卡的 `payload`，**位置 position 尽量不动**。
   - `新增卡片` → 插到指定位置。
   - 这就是 SBIR_NEW「前段 + 新内容 + 后段」字串拼接的**卡片阵列版**。

6. **安全阀（抄 SBIR_NEW 30%）**：若 AI 这次要动的卡片数 / 内容量超过某阈值（例如要改超过一半的卡，或单卡字数暴冲 >30%），**作废这次合并、保留原状**，提示使用者「变动过大，已保留原内容」。

7. **存版本 + 更新指纹 + 重算摘要**：
   - 存一张「全篇卡片快照」（抄 `proposal_versions` 做法，改存卡片阵列 JSON）。
   - 更新系统 `原文指紋` = 新原文 hash；每张被生出的卡更新 `來源原文指紋`。
   - `重新結構化次數 += 1`。
   - （阶段二）重算摘要 + 向量（给全局搜寻用）。

8. **前端只重画变动卡**：其余卡不闪动、不重排。

### 6.3 保护机制（硬需求：100% 不动手改卡）

三层保险，缺一不可：

1. **资料层**：手改/新增的卡标 `source=manual` 或 `locked=true`。
2. **Prompt 层**：明确告诉 AI「这些 id 的卡不准动」，并**根本不把它们的可改权交给 AI**（只给只读上下文）。
3. **合并层**：后端合并时，对 `locked`/`manual` 卡做白名单挡板——即使 AI 回了要改它们，也直接丢弃那条 patch。

> SBIR_NEW 只有第 2 层（不送 AI）；BrainStrom 因为是硬需求，要三层都做。
...
| 3 | 模拟后端 | `web/src/api/mockClient.js` | 实作 `structure()`，补 `source/locked/sourceHash` 栏位，incremental 用简单规则模拟 | （阶段二不用，逻辑搬到 Fly.io） |
...
| splice（字串拼接） | 卡片缝合 | `mergeCards` | 卡片阵列版合并 |
| locked / source=manual | 锁定 / 手改保护 | `locked` / `source` | 永久保护 |
...
| proposal snapshot | 全篇卡片快照 | `cardSnapshot` | 可整份还原 |
...
1. **资料模型**：在卡片（block）加 `source` / `locked` / `sourceAnchor` / `structureGen`；在系统（system）加 `sourceHash` / `aiRestructureCount`。（对应 `backend-design.md` 已规划命名，落到真实 schema / 模拟后端。）
2. **差异侦测层**（纯程式，无 AI）：写「旧原文 vs 新原文 → 变动块清单」的函式；含「原文指纹一样就直接 return 不动作」的 hash gate。
3. **增量 prompt + JSON schema**：照 4.3 措辞写 system prompt；用 `tool_use` 定义「新增卡片 / 更新卡片」的 JSON schema，强制 AI 只回 patch。
4. **卡片缝合层 `mergeCards`**：实作 6.2 步骤 5 + 6.3 三层保护 + 6.4 安全阀。`locked`/`manual` 白名单硬挡。
5. **版本与还原**：抄 `proposal_versions` 做法，改存卡片阵列 JSON（全篇快照 + 可选单卡版本）。
6. **服务层 + 触点 + 前端接线**：照 6.5 表把 `結構化(systemId,{mode})` 串起来，前端只重画变动卡。
7. **测试**（补 SBIR_NEW 没测的）：覆盖「只新增 / 改中段 / 删内容 / 重排 / 动到 locked 卡（必须被挡）/ 原文没变（必须不呼叫 AI）/ AI 超时降级」。
8. **（阶段二）部署到 Fly.io**：把 4～5 的逻辑放进 `POST /ai/structure`，金钥只在 Fly.io，前端只送原文+收 patch。

### 待使用者后续确认（不挡开发，但影响细节）

- 「变动过大安全阀」的阈值要多少（SBIR 用 30% 字数；BrainStrom 卡片版可能用「改超过 X% 的卡」）。
- 单卡版本要不要做（还是只做全篇快照就够）。
...
- BrainStrom 要补这块：你乱改原文后，系统自己用「指纹比对 + 文字 diff」算出哪里变，只把变动的丢给 AI，AI 只回「要新增/要改的卡」，你手改过的卡三层保险绝对不碰，原文没变就完全不花 AI 钱。
