...
> ⚠️ **2026-06-11 改版（同日两次，现行 = v3 活文件模型）**：原第 1 条「手改永久保护」先被推翻（v2 改钉选），v2 的「三层模型（原始散文层永久并存、AI 永不碰）」当天又被 **v3 取代**。**v3 = 活文件模型**：没有永久并存的原始手写层，一份笔记就是一串块（文章/卡片两视图同一份资料）；新增「优化文字」按钮；diff 与省钱的基准从「原始稿 hash」搬到「**上次 AI 输出**」（系统级 `lastAiHash` ＋ 块级 `aiHash`）；安全网 = **无限版本历史 + 不限次数 Undo/Redo**（第 1 版 = 原始乱稿，永远找得回）。详见 `docs/阶段二开发文档-变聪明-AI功能.md` §0.2、§1.2。本文档**上半部（SBIR_NEW 忠实记录）不受影响**；**下半部已按新规则改写**（下文「原文」一律读作「上次 AI 输出时的活文件内容」）。

1. ~~**手改永久保护**（AI 永不动手改卡）~~ → **改为「钉选保护 + 版本还原」**：只有被使用者「钉选（pinned）」的块/卡 AI 永不动；其余（含手动加的）AI 可改、可删；安全网 = 每步操作前自动存版本快照 + 不限次数的上一步/下一步。
2. **按钮触发**：所有 AI 动作由使用者按按钮才跑，不自动侦测自动跑。
...
5. **（v3）活文件模型**：一份笔记 = 一串有顺序的块；「文章」与「卡片」是同一串块的两个视图，不存两份；没有永久并存的原始手写层（最早原稿 = 版本历史第 1 版）；两个 AI 按钮 =「优化文字」（仍是文章）＋「卡片结构化」（归组成卡，共用优化管线）。
...
- `塊指紋`（`aiHash`，v3 取代 `sourceAnchor`）：**上次 AI 处理完时这个块长什么样**的指纹。下次比对：没指纹 = 新写的、指纹不符 = 被改过、上次有这块现在没了 = 被使用者删了。这就是 diff 与「没变不重送」的基准（v3 没有永久原始层，基准从「原始稿」搬到「上次 AI 输出」）。
- `結構化代數`（`structureGen`）：这张卡是第几代 AI 操作产生的。

系统层（system）要有：

- `全文指紋`（`lastAiHash`，v3 取代 `sourceHash` 语意）：上次 AI 操作完成时整份内容的指纹，用来一秒判断「内容到底有没有变」。
- `重新結構化次數`（`ai_restructure_count`）：每跑一次 AI 操作 +1。
- `文件狀態`（`docState`）：raw → optimized → carded。
...
使用者按「卡片结构化」按钮后（第二次以后）：

1. **Hash gate**：把现在的全文算一个 hash，和系统记录的 `lastAiHash` 比。
   - **一样** → 内容没变，**完全不呼叫 AI**，直接显示旧卡片。（这就是「省钱」的第一道闸，SBIR_NEW 没有，我们补上。）
   - **不一样** → 进第 2 步。

2. **差异侦测（SBIR_NEW 缺、我们新做）**：逐块比 `aiHash`，算出哪些块是新增/变动/被删的：
   - 每个块（段落）各自有上次 AI 处理完的指纹。
   - 没指纹的块 =「新写的」；指纹不符 =「被改过」；上次有、现在没了 =「被使用者删了」。
   - 这一步**纯程式、不花 AI 钱**。
   - （v3 说明：活文件里使用者直接在块上写/改/删，所以不再需要「旧原文 vs 新原文」两份文本对比——基准就存在每个块的 `aiHash` 里。）

3. **组增量输入给 AI**：只把「新增/变动块的内容＋被删块的旧文＋现有卡片清单（标注哪些是 `pinned` 钉选）」交给 AI，并下死命令（抄 SBIR_NEW prompt 精神）：
   - 「`pinned`（钉选）的卡绝不准动——不改、不删、不移、不跨越它合并前后文。」
   - 「没变的块已是优化过的成品，**一个字都不准重写**（防影印机效应）。」
   - 「只针对我给你的变动内容，决定要新增哪几张卡、更新哪几张卡；**删除只准用在『内容已被使用者删掉或并入他卡』的卡**。」
...
5. **本地合并（卡片版 Splice）**：在后端用程式把 patch 套到现有卡片阵列：
   ...
   - `刪除卡片` → 只有「内容确实已被使用者删掉或并入他卡」且未钉选的卡才真的删；否则丢弃这条 patch。
   - `新增卡片` → 插到指定位置。
   - 这就是 SBIR_NEW「前段 + 新内容 + 后段」字串拼接的**块阵列版**。

6. **安全阀（抄 SBIR_NEW 30%，加删除规则）**：若 AI 这次「要改＋要删」的卡片数 / 内容量超过某阈值（例如动超过一半的卡，或单卡字数暴冲 >30%），或想动钉选卡/模组卡、想重写没变的块、想删「内容明明还在」的卡，**作废这次合并、保留原状**，提示使用者「变动过大，已保留原内容」。

7. **存版本 + 推 Undo + 更新指纹 + 重算摘要**：
   - 套用前存一张「全篇卡片快照」进 `structure_versions`，并推进 Undo 堆叠——**一次增量 = 一步**，按「上一步」整批撤销、「下一步」整批重做，也可在版本列表一键还原任意旧版。
   - 更新系统 `全文指紋（lastAiHash）` = 当前全文 hash；每张被生出/更新的卡更新 `塊指紋（aiHash）`。
   - `重新結構化次數 += 1`。
   - （阶段二）重算摘要 + 向量（给全局搜寻用）。

8. **前端只重画变动卡**：新增的浮现、更新的原位刷新、删除的淡出；其余卡不闪动、不重排。文章视图与卡片视图**同时生效**（同一串块的两个视图）。
...
- **内容没变就不呼叫 AI**（`lastAiHash` hash gate）—— 新做，最省。「优化文字」和「卡片结构化」两个按钮都走这道闸。
- **只送变动块给 AI**（逐块比 `aiHash`，没变的块一个字不送）—— 抄 SBIR_NEW extract 精神，顺便防「影印机效应」（AI 反复重写同一段、逐代走样）。
- **两按钮共用管线**：刚优化完就按卡片结构化，已优化没变的块直接跳过优化步，不重复扣钱。
...
|---|---|---|---|---|
| 1 | 服务层 | `web/src/services/index.js` | 新增 `optimize(systemId,{groupTopics})` 与 `structure(systemId,{mode})`，打 Mock | 同介面改打 `POST /ai/optimize`、`POST /ai/structure` |
| 2 | 触点登记 | `web/src/touchpoints.js` | 加 `ai.optimize → POST /ai/optimize`、`ai.structure → POST /ai/structure` | 同 |
| 3 | 模拟后端 | `web/src/api/mockClient.js` | 实作 `optimize()/structure()`，补 `source/pinned/aiHash/lastAiHash/docState` 栏位 + body 迁移切块，incremental 用简单规则模拟 | （阶段二不用，逻辑搬到 Fly.io） |
| 4 | 前端 | `web/src/main.js` `renderContent()` | body 大 textarea 换成段落块编辑器；占位字串换成「呼叫服务层 → 渲染卡片、只更新变动卡」 | 介面不变，UI 不用改 |
| 5 | 真后端 | Fly.io AI 代理 `POST /ai/optimize` + `POST /ai/structure` | — | 实作 6.2 全套（块级 diff + AI 合并 + JSON patch + 钉选三层保护 + 安全阀 + 版本 + 快取），两端点共用优化管线 |
...
| sourceHash（系统级，v2 旧名） | 全文指纹 | `lastAiHash` | 上次 AI 操作完成时的全文指纹，hash gate 用 |
| sourceAnchor（卡级，v2 旧名） | 块指纹 | `aiHash` | 上次 AI 处理完时这块的指纹，diff 基准 |
...
5. **版本与还原**：抄 `proposal_versions` 做法改 `structure_versions`（存块阵列 JSON，**无限保留**）；在它上面做 **Undo/Redo 堆叠**（上一步/下一步，**不限次数**；一次 AI 操作 = 一步、改完一张卡 = 一步、打字不算步）+ 版本列表一键还原（只做整篇还原，不做捡回片段）。
6. **服务层 + 触点 + 前端接线**：照 6.5 表把 `optimize(systemId,{groupTopics})` 与 `structure(systemId,{mode})` 串起来；`undo/redo/versions/restore` 也登记触点；前端只重画变动卡，文章/卡片两视图共用同一串块渲染。
7. **测试**（补 SBIR_NEW 没测的）：覆盖「只新增 / 改中段 / 删内容（对应卡要被删）/ 重排 / 动到钉选卡（必须被挡）/ 重写没变的块（必须被挡）/ 删『内容还在』的卡（必须被挡）/ 内容没变（必须不呼叫 AI）/ 连按优化两次（第二次零成本）/ Undo 后 Redo / 一路 Undo 到第 1 版原始稿 / AI 超时降级」。
8. **（阶段二）部署到 Fly.io**：把 4～5 的逻辑放进 `POST /ai/optimize` + `POST /ai/structure`（共用优化管线），金钥只在 Fly.io，前端只送内容+收 patch。

### 待使用者后续确认（不挡开发，但影响细节）

- 「变动过大安全阀」的阈值要多少（SBIR 用 30% 字数；BrainStrom 块版可能用「动＋删超过 X% 的块」）。
- 单卡版本要不要做（还是全篇快照 + Undo/Redo 就够）。
- 增量是否要用便宜模型（Haiku）做省钱分流。

### 已于 2026-06-11 拍板（取代旧悬念；v3）

- **活文件模型**：不保留永久原始层；一份笔记 = 一串块，文章/卡片两视图同一份资料 ✔
- 两个 AI 按钮：「优化文字」＋「卡片结构化」；「要不要分主题」每次都问 ✔
- `locked` 改「钉选 `pinned`」，保护权交使用者自选 ✔
- AI 可删卡（内容被使用者删掉的卡可删；钉选不删）✔
- 经典「上一步/下一步」Undo/Redo，**不限次数**；版本快照无限保留（第 1 版 = 原始乱稿）✔
- 回旧版只做整篇还原，不做捡回片段 ✔
- 模组卡在文章/卡片两视图视觉呈现同一个组件，AI 永不动其内容 ✔
- 段落 = 块（文章每段底层就是一个块，「改卡=改文章」天生成立）✔
...
- 它缺一块：不会自己看出「内容哪里变了」，得你点名。
- BrainStrom 要补这块：每个段落都有「上次 AI 弄完时的指纹」，你改了哪里、删了哪里，系统自己比指纹就知道，只把变动的丢给 AI；AI 只回「要新增/要改/要删的卡」；内容没变就完全不花 AI 钱；没变的段落 AI 一个字都不准重写（防止越改越走样）。
- 你的笔记是「一份会一直进化的活文件」：一串段落＋模组块，看成一篇文章或一张张卡都是同一份资料，改哪边另一边自动一样。两颗按钮：「优化文字」把文章弄顺（还会问你要不要分主题），「卡片结构化」把内容归组成卡。
- AI 什么都能动、连删都行——**除了你亲手「钉选」的，三层保险绝对不碰**。怕改坏？每做一件事系统就自动存档一步，「上一步」可以无限退，退到底就是你最早的原始乱稿，永远找得回。
