...
> ⚠️ **2026-06-11 规则改版（先读这个）**：本文档里所有「100% 不动原本 / 手改卡永不被覆盖 / locked 保护」的说法**已被使用者推翻**。新规则见 `docs/阶段二开发文档-变聪明-AI功能.md` §0.2 与 §1.2「三层内容模型」：原始散文 AI 永不改写；AI 可改、可删一切**未钉选**的卡（含手动卡）；只有使用者「钉选（pinned）」的卡永不碰；安全网 = 版本快照 + 一键还原 + 上一步/下一步（Undo/Redo）。本文档其余背景（SBIR_NEW 研究任务、整合落点）仍可参考，该研究已完成、记录在 `docs/增量结构化-SBIR模组完整记录与开发设计.md`。
